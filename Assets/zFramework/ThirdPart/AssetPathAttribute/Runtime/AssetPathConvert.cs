...
public partial class AssetPath
{
    ...
    public static async UniTask<T> LoadAsync<T>(string resPath) where T:UnityEngine.Object
    {
        if (string.IsNullOrEmpty(resPath))
        {
            return default;
        }
        string resourcesPath = ConvertToResourcesPath(resPath);
        if (!string.IsNullOrEmpty(resourcesPath))
        {
            var obj = await Resources.LoadAsync<T>(resourcesPath) ; //unitask 实现的是直接给结果的
            return obj as T;
        }
        return default;
    }
