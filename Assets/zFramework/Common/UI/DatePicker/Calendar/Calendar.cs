...
namespace SpringGUI
{
    ...
    public class Calendar : UIBehaviour
    {
        ...
        public DateTime SelectDT { get => m_selectDT; set => m_selectDT = value; }
        #endregion
        ...
        private void DayGenerator(GameObject dayPrefab, Transform parent)
        {
            for (int i = 0; i < 42; i++)
            {
                ...
                Text dayText = day.transform.Find("Text").GetComponent<Text>();
                dayText.ChangeTextColor(E_TextColorType.Text_Gray);
                DMY dmy = day.AddComponent<DMY>();
                day.GetComponent<Button>().onClick.AddListener(() =>
                {
                    dayText.ChangeTextColor(E_TextColorType.Black);
        ...
        private void RefreshDays(List<DateTime> dateTimes)
        {
            for (int i = 0; i < _daysPool.Count; i++)
            {
                ...
                _daysPool[i].SetDay(dateTimes[i], DisplayType, SelectDT);
            }
        }
    ...
    public class DMY : UIBehaviour
    {
        ...
        public void SetDay(DateTime dateTime, E_DisplayType displayType/*, Color fontColor*/, DateTime selectTime)
        {
            DateTime = dateTime;
            _text.text = dateTime.Day.ToString();
            if (dateTime.Equals(selectTime))
            {
                _text.ChangeTextColor(E_TextColorType.Black);
                transform.GetComponent<Image>().enabled = true;
            }
            else
            {
                _text.ChangeTextColor(E_TextColorType.Text_Gray);
