...
namespace ChoiceTime
{
    ...
    /// 日期选择器
    /// </summary>
    public class DatePickerDrop : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        /// <summary>
        /// 日期类型 （年月日时分秒）
        /// </summary>
        ...
        GameObject SpawnItem(float real_i)
        {
            ...
            _item.transform.SetParent(_itemParent,false);
            if (real_i != 0)
            {
                _item.GetComponentInChildren<Text>().color = new Color(1, 1, 1, 1 - 0.2f - (Mathf.Abs(real_i) / (_itemNum / 2 + 1)));
            }
        ...
        public void RefreshDateList()
        {
            ...
            for (int i = 0; i < _itemNum; i++)
            {
                ...
                _itemParent.GetChild(i).GetComponentInChildren<Text>().text = str;
            }
