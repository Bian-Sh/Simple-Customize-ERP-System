...
namespace zFrame.Events
{
    public class EventHandler
    {
        ...
        private List<EventInfo> eventContainer =  new List<EventInfo>();
        ...
        public EventInfo GenerAnimationInfo(Animator animator, string clipName)
        {
            AnimationClip clip = GetAnimationClip(animator, clipName);
            if (null == clip)return null;
            EventInfo a_EventInfo = GetEventInfo(animator, clip);  //获取指定事件信息类
            return a_EventInfo;
        }
        ...
        /// <param name="frame">指定帧</param>
        public void GenerAnimationEvent(EventInfo eventInfo, int frame)
        {
        ...
        private EventInfo GetEventInfo(Animator animator, AnimationClip clip)
        {
            EventInfo a_EventInfo = eventContainer.Find((v) => { return v.animator == animator && v.animationClip == clip; });
            if (null == a_EventInfo)
            {
