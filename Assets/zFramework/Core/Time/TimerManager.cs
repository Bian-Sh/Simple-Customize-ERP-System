...
namespace zFrame.Time
{
    public class Timer
    {
        ...
        private Timer(float time, string flag, bool loop = false, bool ignorTimescale = true)
        {
            ...
            if (null == driver&&Application.isPlaying) driver = TimerDriver.Get; //初始化Time驱动 ,只有当非编辑器模式下才会检查驱动
            _time = time;
            _loop = loop;
        ...
        {
            timers.ForEach((v) => { if (null != v) v.Stop(); });
            timers.Clear();
        }
    ...
    public class TimerDriver : MonoBehaviour
    {
        ...
        public static TimerDriver Get
        {
            get
            {
                ...
                if (null==_instance)
                {
                    GameObject go= new GameObject("TimerEntity");
                    _instance=go.AddComponent<TimerDriver>();
