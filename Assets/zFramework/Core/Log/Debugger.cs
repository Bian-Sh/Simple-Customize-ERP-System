...
namespace zFrame.Log
{
    ...
    public class Debugger : MonoSingleton<Debugger>
    {
        ...
        {
            if (!PlayerPrefs.HasKey("zFrame_DebugerStatus"))
            {
                PlayerPrefs.SetInt("zFrame_DebugerStatus", 1);
            }
            AllowDebugging = PlayerPrefs.GetInt("zFrame_DebugerStatus") == 0;
        }
        private void Update()
        {
            if (Input.GetKeyDown(KeyCode.P))
            {
                ...
                PlayerPrefs.SetInt("zFrame_DebugerStatus", AllowDebugging ? 1 : 0);
            }
            if (AllowDebugging)
            {
                _frameNumber += 1;
                float time = UnityEngine.Time.realtimeSinceStartup - _lastShowFPSTime;
                if (time >= 1)
                {
                    _fps = (int)(_frameNumber / time);
                    _frameNumber = 0;
                    _lastShowFPSTime = UnityEngine.Time.realtimeSinceStartup;
                }
            }
        }
